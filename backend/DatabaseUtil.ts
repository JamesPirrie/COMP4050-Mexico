...
export class dbUtils {
    ...
    async getRubricsForAssignments(user_id: number, specificClass: number, specificAssignment: number): Promise<postgres.RowList<postgres.Row[]> | undefined>{
        try {
            const users = user_id;
            if (users) {
                const verifyUser = await sql`SELECT author_id FROM class WHERE class_id = ${specificClass};`;
    ...
    async postRubricForAssignment(user_id: number, assignment_id: number, rubric: string): Promise<Boolean> {
        try {
            const verifyAssignment = await sql`SELECT * FROM assignments WHERE assignment_id = ${assignment_id};`;
            if(verifyAssignment.length < 1){
                throw new Error('No such Assignment found');
     ...
     async deleteStudent(user_id: number, student_id: number): Promise<Boolean> {
        try{     
            ...
            for(var i = 0; i < submissions.length; i++){
                ...
                if(fs.existsSync(`${ROOTDIR}/ServerStorage/PDF_Storage/${filePath}`)){//if theres an entry file for this submission
    ...
    async deleteRubric(user_id: number, rubric_id: number, class_id: number): Promise<Boolean> {
        try{
    ...
    async editRubric(user_id: number, rubric_id: number, assignment_id: number, class_id: number, rubric: string): Promise<Boolean> {
        try{
            const verifyRubric = await sql`SELECT * FROM rubric_output WHERE result_id = ${rubric_id};`;
            if(verifyRubric.length < 1){
                throw new Error('No such assignment found');
