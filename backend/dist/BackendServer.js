...
app.get('/api/classes', async (req, res) => {
    ...
    try {
        console.log('Received GET to /api/classes from: ' + req.query.email + ' = ' + await (0, DatabaseUtil_1.getUserIDbyEmail)(JSON.stringify(req.query.email)));
...
app.post('/api/classes', async (req, res) => {
    ...
    try {
        console.log('Received POST to /api/classes from: ' + req.query.email + ' = ' + await (0, DatabaseUtil_1.getUserIDbyEmail)(JSON.stringify(req.query.email)));
        const success = await (0, DatabaseUtil_5.createClass)(JSON.stringify(req.query.email), JSON.stringify(req.query.code)); // more fields added post MVP
        if (success) {
            res.send('true');
